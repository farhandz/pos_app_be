...
module.exports = {
  ...
  displayAll: (sort, by, search)=> {
    return new Promise((resolve, reject)=> {
      db.query(
        `SELECT produk.id, image, id_category, harga, title, nama_kategori from produk INNER JOIN category on produk.id_category = category.id  WHERE title LIKE '%${search}%'   ORDER BY ${sort}  ${by} `,
        (err, result) => {
          if (err) {
            reject(err);
          } else {
  ...
  getAll: ()=> {
    return new Promise((resolve, reject)=> {
      db.query(`SELECT produk.id, image, id_category, harga, title, nama_kategori from produk INNER JOIN category on produk.id_category = category.id`, (err, result)=> {
        if(err) {
          reject(err)
        } else {
          resolve(result)
