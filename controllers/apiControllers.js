...
module.exports = {
  ...
  detailCheckout: async (req, res) => {
    try {
      const {id} = req.params
      const data = await TransaksiModel.getHistoryDetails(id)
      res.send(data)
    } catch (error) {
      res.send(error.message)
    }
  },
  tanggalCheckout: async(req,res) => {
    try {
      const {tanggal} = req.params
      const data = await TransaksiModel.getCountThisDay(tanggal)
      res.send(data)
    } catch (error) {
      res.send(error.message)
    }
